import { Code2, Copy, FastForward, PanelRightClose, PanelRightOpen , Eye, Flag, Check,X } from 'lucide-react'
...
<body>
 ...
 const handleCopy = async()=>{
    ...
    return (
      <>
        ...
        <div className='h-14 px-4 border-b border-white/6 flex items-center gap-3 shrink-0'>
          <button className='flex items-center justify-center w-7 h-7 rounded-lg text-slate-500 hover:text-slate-200 hover:bg-white/5 
          ...
          <div className='flex items-center gap-1 shrink-0'>
            ...
            hover:bg-white/5 rounded-lg transition-colors duration-150 bg-transparent border-none cursor-pointer' onClick={handleCopy}>
              {copied ? <Check size={15}/> : <Copy size={15}/>}
            </button>
          </div>

          {canPreview && 
            <div className='flex items-center gap-1 bg-white/4 border-white/6 p-1 rounded-lg'>
            <button
            onClick={()=>setTab("code")}
        ...
        { tab === "code" &&
          <div className='flex h-auto border-b border-white/6 overflow-x-auto scrollbar-none [&::-webkit-scrollbar]:hidden shrink-0'>
          {
            artifacts[0]?.files?.map((f,index)=>(
              ...
              className={`px-4 py-2.5 text-[11px] font-medium whitespace-nowrap transition-colors duration-150 border-r border-white/5 relative
              cursor-pointer bg-transparent ${activeFile === index ? "text-indigo-400" :"text-slate-400 hover:text-slate-300"}`}>
              {f?.name}
              {activeFile === index && <div className='absolute bottom-0 right-0 h-0.5 bg-indigo-500 rounded-t-full'/>}
              </button>
            ))
          }
         </div>
  ...
  return (
    ...
    <button onClick={()=>setMobileOpen(true)} className='lg:hidden fixed bottom-24 right-4 z-40 flex items-center gap-2 px-3.5 py-2 rounded-xl bg-indigo-600
    hover:bg-indigo-500 text-whitetext-[12px] font-medium shadow-lg shadow-indigo-500/20 border-none cursor-pointer transition-colors duration-150'>
      <Code2 size={13}/>
      View Code
    </button>
